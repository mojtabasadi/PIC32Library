...
void MFRC522::setSPIConfig() {
	begin1(72000000,1000,MasterSPI,IntEnableSPI,StandardBuffer);		// Init SPI bus
} // End setSPIConfig()
...
void MFRC522::PCD_WriteRegister(	byte reg,		///< The register to write to. One of the PCD_Register enums.
	...
	_LATE2=0;
	transfer1(reg & 0x7E);					// MSB == 0 is for writing. LSB is not used in address. Datasheet section 8.1.2.3.
	transfer1(value);
	_LATE2=1;
...
byte MFRC522::PCD_ReadRegister(	byte reg	///< The register to read from. One of the PCD_Register enums.
	...
	byte value;
	_LATE2=0;
	transfer1(0x80 | (reg & 0x7E));			// MSB == 1 is for reading. LSB is not used in address. Datasheet section 8.1.2.3.
	value = transfer1(0);					// Read the value back. Send 0 to stop reading.
	_LATE2=1;
	return value;
...
void MFRC522::PCD_ReadRegister(	byte reg,		///< The register to read from. One of the PCD_Register enums.
	...
	transfer1(address);					// Tell MFRC522 which address we want to read
	while (index < count) {
		if (index == 0 && rxAlign) { // Only update bit positions rxAlign..7 in values[0]
			...
			byte value = transfer1(address);	
			// Apply mask to both current value of values[0] and the new data in value.
			values[0] = (values[index] & ~mask) | (value & mask);
		}
		else { // Normal case
			values[index] = transfer1(address);	// Read value and tell that we want to read the same address again.
		}
		index++;
	}
	values[index] = transfer1(0);			// Read the final byte. Send 0 to stop reading.
	_LATE2=1;
} // End PCD_ReadRegister()
